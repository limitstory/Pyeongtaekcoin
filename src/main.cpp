...
#if defined(NDEBUG)
# error "Pyeongtaekcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x5448311c83e77ba5687f1fb54e3099adba605239b103ddd3c84cdcedfc7b1010");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Pyeongtaekcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Pyeongtaekcoin Signed Message:\n";
...
{
    // Pyeongtaekcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000000); // Pyeongtaekcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Pyeongtaekcoin: 1 days
static const int64 nTargetSpacing = 1 * 60; // Pyeongtaekcoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Pyeongtaekcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Pyeongtaekcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xa27ceb787473959a24da88b2586ac770ba4b5df91ea0907b707c6cd358f071d6");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Pyeongtaek Coin 2020-02-24, Pyeongtaek Univ.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0438b5c55b6b65d93ce6f604fe00782b4bd148188179adbd77c0604cff8c84311c5b9922becbc5e79097453768bbd0852fa73ed6785bb46214e6f0ba2ff772c018") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1582543698;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085897069;

        if (fTestNet)
        {
            block.nTime    = 1582543603;
            block.nNonce   = 386173995;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x027cfd67f6da8d51c3651b388b055912fb978771327ec9d84cb2cad5120214ce"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Pyeongtaekcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PyeongtaekcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PyeongtaekcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PyeongtaekcoinMiner(CWallet *pwallet)
{
    printf("PyeongtaekcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("pyeongtaekcoin-miner");
    ...
    try { loop {
        ...
        printf("Running PyeongtaekcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PyeongtaekcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PyeongtaekcoinMiner, pwallet));
}
