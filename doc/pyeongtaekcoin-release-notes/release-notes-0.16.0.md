Pyeongtaekcoin Core version 0.16.0 is now available from:

  <https://download.pyeongtaekcoin.org/pyeongtaekcoin-0.16.0/>
...
### BIP173 (Bech32) Address support ("ptc1..." addresses)
...
Pyeongtaekcoin Core now has more flexibility in where the wallets directory can be
located. Previously wallet database files were stored at the top level of the
pyeongtaekcoin data directory. The behavior is now:
...
------------------------------------
The minimum version of the GCC compiler required to compile Pyeongtaekcoin Core is now 4.8. No effort will be
...
* A new field `iswitness` is True for P2WPKH and P2WSH addresses ("ptc1..." addresses), but not for P2SH-wrapped segwit addresses (see below).
...
- pyeongtaekcoin-cli now has an `-stdinrpcpass` option to allow the RPC password to be read from standard input.
- The `-usehd` option has been removed.
- pyeongtaekcoin-cli now supports a new `-getinfo` flag which returns an output like that of the now-removed `getinfo` RPC.

Testing changes
----------------
- The default regtest JSON-RPC port has been changed to 19443 to avoid conflict with testnet's default of 13332.
- Segwit is now always active in regtest mode by default. Thus, if you upgrade a regtest node you will need to either -reindex or use the old rules by adding `vbparams=segwit:0:999999999999` to your regtest pyeongtaekcoin.conf. Failure to do this will result in a CheckBlockIndex() assertion failure that will look like: Assertion `(pindexFirstNeverProcessed != nullptr) == (pindex->nChainTx == 0)' failed.
...
- #11917 `bc66765` Add testnet DNS seed:  seed.testnet.pyeongtaekcoin.sprovoost.nl (Sjors)
...
- #11970 `b7450cd` Add test coverage for pyeongtaekcoin-cli multiwallet calls (ryanofsky)
...
- #10997 `affe927` Add option -stdinrpcpass to pyeongtaekcoin-cli to allow RPC password to be read from standard input (jharvell)
...
- #12035 `eeb6d52` Change µPTC to bits (jb55)
...
- #11621 `1f7695b` [build] Add temp_pyeongtaekcoin_locale_qrc to CLEAN_QT to fix make distcheck (fanquake)
...
- #11711 `6378e5c` pyeongtaekcoin_qt.m4: Minor fixes and clean-ups (fanquake)
...
- #11297 `16e4184` Make sure ~/.pyeongtaekcoin doesn't exist before build (MeshCollider)
...
- #11143 `3aa60b7` Fix include path for pyeongtaekcoin-config.h (danra)
...
- #10871 `c9a4aa8` Handle getinfo in pyeongtaekcoin-cli w/ -getinfo (revival of #8843) (achow101)
- #11419 `093074b` Utils: Fix launchctl not being able to stop pyeongtaekcoind (OmeGak)
...
- #10529 `331352f` Improve pyeongtaekcoind systemd service file (Flowdalic)
...
- #482 `5c8e26d` Pyeongtaekcoin: Basic changes for v0.16 release (thrasher-)
